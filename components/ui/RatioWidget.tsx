    ...
    <div className="flex flex-wrap gap-3 text-xs text-slate-500 dark:text-slate-400">
      {items.map(({ key, label }) => (
        <span key={key} className="inline-flex items-center gap-1.5">
          ...
          {total > 0 && <span className="text-slate-400">({Math.round((counts[key] / total) * 100)}%)</span>}
        </span>
      ))}
    </div>
  );
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {scopeLabel && <span className="text-xs text-slate-400">{scopeLabel}</span>}
...
/** Detail Demand's "Rasio Aktual — bisa dibandingkan skenario": 3 bars
 * (Sekarang / diganti Kontrak baru / diganti MP Excess Permanen), each a
 * full RatioWidget-style bar+legend so the shift in composition is visible
 * scenario by scenario. Callers compute each scenario's counts themselves
 * (e.g. by re-tallying with the demand's outgoing/replacement swapped) —
 * this component only lays them out side by side. */
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">
          Rasio Aktual — bisa dibandingkan skenario
        </h3>
        {scopeLabel && <span className="text-xs text-slate-400">{scopeLabel}</span>}
      </div>
      <div className="space-y-3">
        {scenarios.map((s) => (
          <div key={s.label}>
            ...
            {s.hint && <div className="mt-1 text-[11px] text-slate-400">{s.hint}</div>}
          </div>
        ))}
      </div>
